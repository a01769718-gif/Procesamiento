...
%
% Correcciones:
% 1. Ya no falla si el CSV no tiene columna "filename".
% 2. Recupera dx, dy y dz desde el archivo NIfTI si el CSV no los trae.
% 3. Mantiene compatibilidad con la Fase 5.
% 4. Permite seleccionar manualmente ambos ventrículos.
%% ============================================================
...
overlayFase3Path = fullfile(phase3Folder, ...
...
%% Leer tabla de Fase 3

T3 = readtable(csvFase3, ...
    'TextType', 'string', ...
    'VariableNamingRule', 'preserve');

if isempty(T3)
    error('El CSV de Fase 3 está vacío.');
end

%% Buscar archivo NIfTI original

niiPath = fullfile(rootFolder, targetFile);

if ~exist(niiPath, 'file')

    niiFiles = dir(fullfile(rootFolder, '**', targetFile));

    if isempty(niiFiles)
        warning('No se encontró %s. Se intentará continuar solo con los datos del CSV.', targetFile);
        niiPath = "";
    else
        niiPath = string(fullfile(niiFiles(1).folder, niiFiles(1).name));
    end

else

    niiPath = string(niiPath);

end

%% Recuperar resolución espacial

dx = getTableValue(T3, ["dx_mm", "dx", "PixelDimensions_1", "pixelX_mm"]);
dy = getTableValue(T3, ["dy_mm", "dy", "PixelDimensions_2", "pixelY_mm"]);
dz = getTableValue(T3, ["dz_mm", "dz", "PixelDimensions_3", "sliceThickness_mm"]);

if isnan(dx) || isnan(dy) || dx <= 0 || dy <= 0 || isnan(dz) || dz <= 0

    if strlength(niiPath) > 0 && exist(niiPath, 'file')
        ...
        if isfield(info, 'PixelDimensions') && numel(info.PixelDimensions) >= 3
            ...
            fprintf('\nResolución recuperada desde NIfTI:\n');
            fprintf('dx = %.4f mm/px\n', dx);
            fprintf('dy = %.4f mm/px\n', dy);
            fprintf('dz = %.4f mm\n', dz);
...
if isnan(dx) || dx <= 0
    warning('No hay dx válido. Se usará dx = 1 mm/px.');
    dx = 1;
end

if isnan(dy) || dy <= 0
    warning('No hay dy válido. Se usará dy = 1 mm/px.');
    dy = 1;
end

if isnan(dz) || dz <= 0
    warning('No hay dz válido. Se usará dz = 1 mm.');
    dz = 1;
end

pixelSizeMean = mean([dx, dy]);

%% Recuperar nombre/ruta del archivo

filenameValue = getTableStringValue(T3, ...
    ["filename", "fileName", "file", "niiPath", "rutaArchivo", "archivo"]);

if strlength(filenameValue) == 0
    filenameValue = niiPath;
end

if strlength(filenameValue) == 0
    filenameValue = string(targetFile);
end
...
if exist(originalPath, 'file')
    ...
    if strlength(niiPath) == 0 || ~exist(niiPath, 'file')
        error('No se encontró la imagen original de Fase 3 ni el NIfTI original.');
    end

    V = double(squeeze(niftiread(niiPath)));

    if ndims(V) > 3
        V = V(:,:,:,1);
    end

    if targetSlice < 1 || targetSlice > size(V,3)
        error('El corte %d no existe en el volumen.', targetSlice);
    end

    I = V(:,:,targetSlice);
    I = normalizar01(I);

    imwrite(uint8(255 * I), originalPath);

end

%% Leer overlay de Fase 3 como referencia, si existe

if exist(overlayFase3Path, 'file')
    overlayFase3 = imread(overlayFase3Path);
else
    overlayFase3 = [];
end

%% ============================================================
% PASO 1: Dibujar los dos ventrículos reales
%% ============================================================
...
for v = 1:2
    ...
    if isempty(overlayFase3)
        ...
        subplot(1,2,1);
        imshow(I, []);
        title('Original para dibujar', 'FontWeight', 'bold');
        ax1 = gca;
        hold(ax1, 'on');

        subplot(1,2,2);
        imshow(overlayFase3);
        title('Referencia Fase 3', 'FontWeight', 'bold');
    ...
    title(ax1, {['Dibuja SOLO el ', char(ventNames(v))], ...
           'Selecciona el ventrículo real; doble clic para cerrar'}, ...
           'FontSize', 13, ...
           'FontWeight', 'bold');

    h = drawfreehand(ax1, 'Color', 'yellow', 'LineWidth', 2);
    BW = createMask(h);
...
for v = 1:2
    ...
    title({['Dibuja el diámetro atrial de ', char(ventNames(v))], ...
           'Traza una línea de borde a borde del ventrículo'}, ...
           'FontSize', 13, ...
           'FontWeight', 'bold');
...
volumenTotalEstimadoCorte_mm3 = areaTotal_mm2 * dz;
...
maskV1Path = fullfile(phase4Folder, ...
    'FaseIV_mascara_manual_ventriculo1_t2_t25_00_corte93.png');

maskV2Path = fullfile(phase4Folder, ...
    'FaseIV_mascara_manual_ventriculo2_t2_t25_00_corte93.png');

maskTotalPath = fullfile(phase4Folder, ...
    'FaseIV_mascara_manual_dos_ventriculos_t2_t25_00_corte93.png');
...
%% ============================================================
% Visual principal de cuantificación
%% ============================================================

visualPath = fullfile(phase4Folder, ...
    'FaseIV_visual_cuantificacion_dos_ventriculos_t2_t25_00_corte93.png');
...
texto = {
    ...
    ''
    ['Archivo: ', char(filenameValue)]
    ''
    ['Pixel: ', num2str(dx, '%.4f'), ' × ', num2str(dy, '%.4f'), ' mm']
    ['Espesor dz: ', num2str(dz, '%.4f'), ' mm']
    ...
    ['Volumen estimado corte: ', num2str(volumenTotalEstimadoCorte_mm3, '%.2f'), ' mm³']
    ''
...
%% ============================================================
% Visual de conversión px -> mm
%% ============================================================

conversionPath = fullfile(phase4Folder, ...
    'FaseIV_conversion_px_mm_dos_ventriculos_t2_t25_00_corte93.png');
...
%% ============================================================
% Guardar CSV resumen para Fase 5
%% ============================================================

Tresumen = table();

Tresumen.caseName = string(targetCase);
Tresumen.filename = string(filenameValue);
...
Tresumen.volumenTotalEstimadoCorte_mm3 = volumenTotalEstimadoCorte_mm3;
...
txtPath = fullfile(phase4Folder, ...
    'FaseIV_resumen_dos_ventriculos_t2_t25_00_corte93.txt');
...
fprintf('\n====================================================\n');
fprintf('FASE 4 CORREGIDA TERMINADA\n');
fprintf('Caso: %s\n', targetCase);
fprintf('Corte: %d\n', targetSlice);
fprintf('Archivo: %s\n', filenameValue);
fprintf('dx = %.4f mm/px | dy = %.4f mm/px | dz = %.4f mm\n', dx, dy, dz);
fprintf('Diámetro V1: %.2f mm\n', diametro_mm(1));
fprintf('Diámetro V2: %.2f mm\n', diametro_mm(2));
fprintf('Diámetro máximo: %.2f mm\n', diametroAtrialMax_mm);
fprintf('Diagnóstico preliminar: %s\n', diagnostico);
fprintf('Área total: %.2f mm²\n', areaTotal_mm2);
fprintf('Volumen estimado corte: %.2f mm³\n', volumenTotalEstimadoCorte_mm3);
fprintf('CSV por ventrículo:\n%s\n', csvVentriculos);
fprintf('CSV resumen para Fase 5:\n%s\n', csvResumen);
fprintf('Visual cuantificación:\n%s\n', visualPath);
fprintf('Visual conversión:\n%s\n', conversionPath);
fprintf('Resumen TXT:\n%s\n', txtPath);
fprintf('====================================================\n');
...
function value = getTableValue(T, possibleNames)
...
function value = getTableStringValue(T, possibleNames)
